...
use pg_auto_explain_analyze_rs::parsing_helper::ParsingHelper;
use std::io::Write;
...
fn create_sample_log_data(num_queries: usize) -> String {
    ...
    for i in 0..num_queries {
        ...
        log_content.push_str(&format!(
            ...
            i % 60, (i * 2) % 60, process_id, duration
        ));
        ...
        if i % 3 == 0 {
            log_content.push_str(&format!(
                ...
                i % 60, (i * 2) % 60, process_id, query_id, query_id * 100
            ));
        }
...
fn bench_original_parser(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000, 2000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("queries", size),
            size,
            |b, _| {
                let parser = PostgreSQLLogParser::new();
                b.iter(|| {
                    let result = parser.parse_file(black_box(&temp_path));
                    black_box(result)
                });
            },
        );
...
fn bench_optimized_parser(c: &mut Criterion) {
    let mut group = c.benchmark_group("optimized_parser");
    ...
    for size in [100, 500, 1000, 2000].iter() {
        ...
        group.bench_with_input(
            ...
            size,
            |b, _| {
                let parser = ParsingHelper::new();
                b.iter(|| {
                    let result = parser.parse_file_optimized(black_box(&temp_path), |_| {});
                    black_box(result)
                });
            },
        );
...
fn bench_parser_comparison(c: &mut Criterion) {
    ...
    let log_content = create_sample_log_data(1000);
    let mut temp_file = NamedTempFile::new().unwrap();
    temp_file.write_all(log_content.as_bytes()).unwrap();
    let temp_path = temp_file.path().to_path_buf();
    
    group.bench_function("original_1000", |b| {
        let parser = PostgreSQLLogParser::new();
        b.iter(|| {
            let result = parser.parse_file(black_box(&temp_path));
            black_box(result)
...
fn bench_string_operations(c: &mut Criterion) {
    ...
    let sample_lines: Vec<String> = (0..1000)
        .map(|i| format!("2024-01-01 10:00:{:02}.123 UTC [{}] LOG:  Some log message with data {}", i % 60, 1000 + i, i))
        .collect();
...
fn bench_regex_operations(c: &mut Criterion) {
    ...
    let parser = PostgreSQLLogParser::new();
    let sample_lines: Vec<String> = (0..1000)
        .map(|i| format!("2024-01-01 10:00:{:02}.123 UTC [{}] LOG:  duration: {:.3} ms  plan:", i % 60, 1000 + i, i as f64 * 0.5 + 10.0))
        .collect();
...
fn bench_memory_usage(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("with_statistics", size),
            size,
            |b, _| {
                let parser = PostgreSQLLogParser::new();
                b.iter(|| {
                    let queries = parser.parse_file(black_box(&temp_path)).unwrap();
                    let statistics = parser.get_query_statistics(&queries);
                    black_box((queries, statistics))
                });
            },
        );
        
        group.bench_with_input(
            BenchmarkId::new("parsing_only", size),
            size,
            |b, _| {
                let parser = PostgreSQLLogParser::new();
                b.iter(|| {
