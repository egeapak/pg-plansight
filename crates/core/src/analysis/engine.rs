...
use super::{Analyzer, AnalysisContext, AnalysisReport, CombinedAnalysisResult, ConfigurableAnalyzer};
...
impl AnalysisEngine {
    ...
    pub fn analyze_with(&self, analyzer_name: &str, plan: &ParsedPlan, context: &AnalysisContext) -> Option<AnalyzerResult> {
        self.analyzers
            .iter()
    ...
    pub fn analyze_filtered<F>(&self, plan: &ParsedPlan, context: &AnalysisContext, filter: F) -> EngineResult
    where
    ...
    fn run_single_analyzer(&self, analyzer: &dyn Analyzer, plan: &ParsedPlan, context: &AnalysisContext) -> AnalyzerResult {
    ...
    pub fn analyzer_stats(&self) -> AnalyzerStats {
        ...
        for analyzer in &self.analyzers {
            stats.total_count += 1;
            stats.analyzer_versions.insert(
                analyzer.name().to_string(),
...
impl EngineResult {
    ...
    pub fn successful_results(&self) -> Vec<&AnalyzerResult> {
        self.analyzer_results.iter().filter(|r| r.is_success()).collect()
    }
    
    /// Get failed analyzer results
    pub fn failed_results(&self) -> Vec<&AnalyzerResult> {
        self.analyzer_results.iter().filter(|r| r.is_error()).collect()
    }
    ...
    pub fn execution_summary(&self) -> ExecutionSummary {
        ...
        let total_analyzer_time: Duration = self.analyzer_results
            .iter()
            .map(|r| r.duration)
            .sum();
    ...
    pub fn has_critical_issues(&self) -> bool {
        use super::Severity;
        self.combined_result.summary.severity_counts
...
mod tests {
    ...
    impl Analyzer for MockAnalyzer {
        fn analyze(&self, _plan: &ParsedPlan, _context: &AnalysisContext) -> AnalysisReport {
            ...
            AnalysisReport::new(self.name.to_string())
