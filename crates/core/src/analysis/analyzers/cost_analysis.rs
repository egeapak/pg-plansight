...
impl Analyzer for CostAnalyzer {
    fn analyze(&self, plan: &ParsedPlan, context: &AnalysisContext) -> AnalysisReport {
        ...
        report = report
            ...
            .with_metric(
                "expensive_operations_found",
                visitor.expensive_operations as f64,
            )
            .with_metric(
                "high_startup_costs_found",
                visitor.high_startup_costs as f64,
            )
...
impl<'a> CostAnalysisVisitor<'a> {
    ...
    fn analyze_expensive_operation(&mut self, node: &PlanNode, path: &NodePath) {
        if !self
            .config
            .enabled_findings
            .contains(&FindingType::ExpensiveOperation)
        {
            return;
        }
    ...
    fn analyze_startup_cost(&mut self, node: &PlanNode, path: &NodePath) {
        if !self
            .config
            .enabled_findings
            .contains(&FindingType::HighStartupCost)
        {
            return;
        }
