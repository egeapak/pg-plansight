...
impl Analyzer for ParallelizationAnalyzer {
    fn analyze(&self, plan: &ParsedPlan, context: &AnalysisContext) -> AnalysisReport {
        let mut report = AnalysisReport::new("ParallelizationAnalyzer".to_string())
            ...
            .with_metadata("max_parallel_workers", &context.max_parallel_workers.to_string());
        ...
        report = report
            ...
            .with_metric("sequential_operations", visitor.sequential_operations as f64)
            .with_metric("missed_parallelization_opportunities", visitor.missed_opportunities as f64)
            .with_metric("inefficient_parallelism", visitor.inefficient_parallelism as f64);
...
impl<'a> ParallelizationVisitor<'a> {
    ...
    fn analyze_parallelization_opportunity(&mut self, node: &PlanNode, path: &NodePath) {
        if !self.config.enabled_findings.contains(&FindingType::MissedParallelization) {
            return;
        }
        ...
        if is_parallel {
            ...
            let cost_severity = self.config.thresholds.costs.classify(&node.cost.max_total_cost);
            
            let severity = std::cmp::max(row_severity, cost_severity);
            
            if matches!(severity, Severity::High | Severity::Critical) && self.could_be_parallelized(node) {
    ...
    fn analyze_parallel_efficiency(&mut self, node: &PlanNode, path: &NodePath) {
        if !self.config.enabled_findings.contains(&FindingType::InefficientParallelism) {
            return;
        }
