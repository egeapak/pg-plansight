...
impl Analyzer for RowEstimationAnalyzer {
    fn analyze(&self, plan: &ParsedPlan, context: &AnalysisContext) -> AnalysisReport {
        ...
        report = report
            ...
            .with_metric("excessive_row_operations", visitor.excessive_row_operations as f64)
    ...
    fn detect_cartesian_product(&mut self, node: &PlanNode, path: &NodePath) {
        ...
        if let crate::NodeType::Join(crate::JoinType::NestedLoop { .. }) = &node.node_type {
            ...
            if !has_join_filter && estimated_rows > 10000 {
                let left_rows = node.children.get(0).map(|c| c.cost.estimated_rows).unwrap_or(1);
                let right_rows = node.children.get(1).map(|c| c.cost.estimated_rows).unwrap_or(1);
...
mod tests {
    ...
    fn test_excessive_row_detection() {
        ...
        let node = PlanNode::new(
            NodeType::Scan(crate::ScanType::SeqScan { 
                table: crate::TableReference { schema: None, name: "large_table".to_string(), alias: None } 
            }),
            PlanCost {
                startup_cost: 0.0,
                min_total_cost: 0.0,
        ...
        assert!(report.findings.iter().any(|f| matches!(f.finding_type, FindingType::ExcessiveRowProcessing)));
    }
    ...
    fn test_cartesian_product_detection() {
        ...
        let left_child = PlanNode::new(
            NodeType::Scan(crate::ScanType::SeqScan { 
                table: crate::TableReference { schema: None, name: "table1".to_string(), alias: None } 
            }),
        ...
        let right_child = PlanNode::new(
            NodeType::Scan(crate::ScanType::SeqScan { 
                table: crate::TableReference { schema: None, name: "table2".to_string(), alias: None } 
            }),
            PlanCost { startup_cost: 0.0, min_total_cost: 0.0, max_total_cost: 100.0, estimated_rows: 500, estimated_width: 50 },
        ...
        assert!(report.findings.iter().any(|f| matches!(f.finding_type, FindingType::CartesianProduct)));
    }
}