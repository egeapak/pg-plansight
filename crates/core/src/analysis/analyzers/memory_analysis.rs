...
use crate::{JoinType, NodeType, ParsedPlan, PlanNode, UtilityType};
...
impl Analyzer for MemoryAnalyzer {
    fn analyze(&self, plan: &ParsedPlan, context: &AnalysisContext) -> AnalysisReport {
        ...
        report = report
            ...
            .with_metric(
                "aggregate_operations_found",
                visitor.aggregate_operations as f64,
            )
...
impl<'a> MemoryAnalysisVisitor<'a> {
    ...
    fn analyze_sort_operation(&mut self, node: &PlanNode, path: &NodePath) {
        if !self
            .config
            .enabled_findings
            .contains(&FindingType::LargeSort)
        {
            return;
        }
        ...
        if matches!(
            ...
            let suggestion = if memory_spill_ratio > 1.0 {
                ...
                "Monitor sort performance and consider optimization if this query runs frequently"
                    .to_string()
            };

            let finding = Finding::new(finding_type, severity, title, description, suggestion)
    ...
    fn analyze_hash_operation(&mut self, node: &PlanNode, path: &NodePath) {
        ...
        if !self
            .config
            .enabled_findings
            .contains(&FindingType::MemorySpill)
        {
            return;
        }
    ...
    fn analyze_aggregate_operation(&mut self, node: &PlanNode, path: &NodePath) {
        if !self
            .config
            .enabled_findings
            .contains(&FindingType::LargeAggregation)
        {
            return;
        }
