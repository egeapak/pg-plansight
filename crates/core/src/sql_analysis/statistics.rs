...
impl StatisticalCalculator {
    ...
    pub fn sample_variance(&self, values: &[f64]) -> f64 {
        ...
        values.iter()
            .map(|x| (x - mean).powi(2))
            .sum::<f64>() / (values.len() - 1) as f64  // N-1 for sample variance
    }
    ...
    pub fn welch_t_test(&self, sample1: &[f64], sample2: &[f64]) -> Result<TTestResult> {
        if sample1.len() < 2 || sample2.len() < 2 {
            return Err(anyhow::anyhow!("Both samples must have at least 2 observations"));
        }
    ...
    pub fn normality_test(&self, values: &[f64]) -> Result<NormalityTestResult> {
        if values.len() < 8 {
            return Err(anyhow::anyhow!("Need at least 8 observations for normality test"));
        }
    ...
    pub fn detect_anomalies_modified_zscore(&self, values: &[f64], threshold: f64) -> Vec<AnomalyInfo> {
        if values.is_empty() {
            return Vec::new();
        }
