...
use pg_loganalyze_core::{PostgreSQLLogParser, QueryPlan, ProcessedQuery, calculate_query_hash};
...
impl LogCollector {
        ...
        let filter_patterns = if let Some(ref filters) = config.filters {
            if let Some(ref patterns) = filters.exclude_query_patterns {
                let compiled_patterns: Result<Vec<_>> = patterns
                    .iter()
                    .map(|pattern| Regex::new(pattern).with_context(|| format!("Invalid regex pattern: {}", pattern)))
                    .collect();
    ...
    pub async fn collect_metrics(&mut self) -> Result<()> {
        let _timer = self.metrics.export_duration.with_label_values(&["collect"]).start_timer();
        ...
        for log_path in log_paths {
            match self.process_log_file(&log_path).await {
                Ok(processed) => {
                    ...
                    self.metrics.logs_parsed_total
                ...
                Err(e) => {
                    ...
                    self.metrics.parse_errors_total
        ...
        info!("Collection complete: {} entries processed, {} files had errors", 
    ...
    pub async fn collect_remaining_metrics(&mut self) -> Result<()> {
        let _timer = self.metrics.export_duration.with_label_values(&["collect_remaining"]).start_timer();
        
        let log_paths = self.expand_log_paths()?;
        info!("Processing remaining content from {} log files", log_paths.len());
        ...
        for log_path in log_paths {
            match self.process_remaining_content(&log_path).await {
                Ok(processed) => {
                    if processed > 0 {
                        ...
                        self.metrics.logs_parsed_total
                            .with_label_values(&[&log_path.to_string_lossy(), "success"])
                            .inc_by(processed as f64);
                        info!("Processed {} remaining lines from {}", processed, log_path.display());
                    } else {
                ...
                Err(e) => {
                    total_errors += 1;
                    error!("Failed to process remaining content from {}: {}", log_path.display(), e);
                    self.metrics.parse_errors_total
                        .with_label_values(&[&log_path.to_string_lossy(), "file_error"])
                        .inc();
                }
        ...
        info!("Remaining content processing complete: {} entries from {} files, {} files had errors", 
    ...
    async fn process_log_file(&mut self, log_path: &Path) -> Result<usize> {
        ...
        let mut file_state = self.state_manager.get_file_state(log_path)?
        ...
        if current_size < file_state.file_size {
            info!("File {} appears to have been truncated/rotated, processing from beginning", 
                  log_path.display());
        ...
        let lines_processed = if new_content_size > 0 {
            ...
            match self.log_parser.parse_file_range_with_progress(log_path, file_state.last_position, end_pos, |_, _| {}) {
                Ok(query_plans) => {
                    if !query_plans.is_empty() {
                        self.process_query_plans(&query_plans).await?;
                        info!("Processed {} query plans from {} bytes of new content in {}", 
                              query_plans.len(), new_content_size, log_path.display());
                ...
                Err(e) => {
                    warn!("Failed to process new content from {}: {}", log_path.display(), e);
                    self.metrics.parse_errors_total
    ...
    async fn process_remaining_content(&mut self, log_path: &Path) -> Result<usize> {
        ...
        let (start_position, should_process) = match file_state {
            Some(state) => {
                ...
                if current_size <= state.file_size {
                    debug!("No remaining content in {} (current: {}, last: {})", 
                           log_path.display(), current_size, state.file_size);
                    return Ok(0);
                }
                info!("Found remaining content in {}: {} bytes (from position {} to {})", 
                      log_path.display(), current_size - state.file_size, state.file_size, current_size);
                (state.last_position, true)
            },
            None => {
                info!("No previous state for {}, processing entire file", log_path.display());
                (0, true)
            }
        };
        ...
        if lines_processed > 0 {
            ...
            match self.log_parser.parse_file_range_with_progress(log_path, start_position, end_pos, |_, _| {}) {
                Ok(query_plans) => {
                    if !query_plans.is_empty() {
                        self.process_query_plans(&query_plans).await?;
                        info!("Successfully processed {} query plans from remaining content", query_plans.len());
                    }
                }
                Err(e) => {
                    warn!("Failed to process remaining content from {}: {}", log_path.display(), e);
                    self.metrics.parse_errors_total
                        .with_label_values(&[&log_path.to_string_lossy(), "parse_error"])
                        .inc();
                }
    ...
    async fn process_query_plans(&mut self, query_plans: &[QueryPlan]) -> Result<()> {
        ...
        for (_query_hash, query) in processed_queries.iter() {
            ...
            self.state_manager.record_query_hash(&stable_hash, &query.normalized_query)?;
            
            // Update metrics
            self.update_query_metrics(&stable_hash, &query_timestamp, &database, query).await?;
        }
    ...
    async fn update_query_metrics(
        ...
        for threshold_str in &self.config.metrics.slow_query_thresholds {
            ...
            let slow_count = query.statistics.executions
                .iter()
        ...
        if let Some(ref parsed_plan) = query.parsed_plan {
            ...
            if let Some(cost) = self.extract_plan_cost(&query.plan) {
                self.metrics.query_plan_cost
                    .with_label_values(labels)
                    .observe(cost);
            }
            
            // Count plan node types
            self.update_plan_metrics(database, query_timestamp, &query.plan).await?;
        }
