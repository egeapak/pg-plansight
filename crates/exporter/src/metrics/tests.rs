...
mod tests {
    ...
    fn test_prometheus_backend_creation() {
        ...
        assert!(metrics.iter().any(|m| m.get_name() == "test_query_duration_seconds"));
        assert!(metrics.iter().any(|m| m.get_name() == "test_query_executions_total"));
    }
    ...
    fn test_prometheus_backend_plan_metrics() {
        ...
        let metrics = backend.registry.gather();
        assert!(metrics.iter().any(|m| m.get_name() == "test_query_plan_cost"));
        assert!(metrics.iter().any(|m| m.get_name() == "test_query_rows_examined"));
    }
    ...
    fn test_helper_functions() {
        ...
        #[cfg(feature = "prometheus")]
        {
            ...
            let metrics = backend.registry.gather();
            let timestamp_metric = metrics.iter()
