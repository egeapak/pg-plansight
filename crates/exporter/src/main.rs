...
use pg_loganalyze_exporter::{Config, StateManager, MetricsRegistry, LogCollector, Scheduler};
...
enum Commands {
    ...
    ProcessRest {
        #[arg(long, help = "Log file paths or patterns (optional, uses config if not specified)")]
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Daemon => {
            run_daemon(config, state_manager).await
        }
        Commands::State { action } => {
            run_state_command(action, state_manager).await
        }
        Commands::Process { logs } => {
            run_process_command(config, state_manager, logs).await
        }
        ...
        StateAction::Show => {
            ...
            if states.is_empty() {
                ...
                for (path, state) in states {
                    println!("  {}: position={}, size={}, last_processed={}",
                             path.display(),
                             state.last_position,
                             state.file_size,
